<div class="content-wrap card auto-height">
    ...
    <div class="text-right">
        <div component="dropdown" class="dropdown-container">
            <button refs="dropdown@toggle" class="button outline" aria-haspopup="true" aria-expanded="false">{{ trans('entities.convert_record') }}</button>
            <ul refs="dropdown@menu" class="dropdown-menu" role="menu">
                <li class="px-m py-s text-small text-muted">
                    {{ trans('entities.convert_record_confirm') }}
                    <br>
                    {{ trans('entities.convert_undo_warning') }}
                </li>
                <li>
                    <form action="{{ $record->getUrl('/convert-to-shelf') }}" method="POST">
                        {!! csrf_field() !!}
