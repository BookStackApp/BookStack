...
class RecordQueries implements ProvidesEntityQueries
{
    ...
    public function findVisibleById(int $id): ?Record
    {
        return $this->start()->scopes('visible')->find($id);
    }

    public function findVisibleByIdOrFail(int $id): Record
    {
        return $this->start()->scopes('visible')->findOrFail($id);
    }

    public function findVisibleBySlugOrFail(string $slug): Record
    {
        /** @var ?Record $record */
        $record = $this->start()
            ->scopes('visible')
        ...
        if ($record === null) {
            throw new NotFoundException(trans('errors.record_not_found'));
        }
