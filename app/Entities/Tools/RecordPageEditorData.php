...
class RecordPageEditorData
{
    protected array $viewData;
    protected array $warnings;

    public function __construct(
        protected RecordPage $page,
    ...
    protected function build(): array
    {
        ...
        $this->warnings = [];
        $editActivity = new RecordPageEditActivity(RecordPage::find($page->id));
        ...
        return [
            ...
            'templates'       => $templates,
            'editor'          => $editorType,
            'comments'        => new RecordCommentTree(RecordPage::find($page->id)),
        ];
    }

    protected function updateContentForEditor(RecordPage $page, RecordPageEditorType $editorType): void
    {
        $isHtml = !empty($page->html) && empty($page->markdown);
        
        // HTML to markdown-clean conversion
        if ($editorType === RecordPageEditorType::Markdown && $isHtml && $this->requestedEditor === 'markdown-clean') {
     ...
     */
    protected function getEditorType(RecordPage $page): RecordPageEditorType
    {
        $editorType = RecordPageEditorType::forPage($page) ?: RecordPageEditorType::getSystemDefault();

        // Use requested editor if valid and if we have permission
        $requestedType = RecordPageEditorType::fromRequestValue($this->requestedEditor);
