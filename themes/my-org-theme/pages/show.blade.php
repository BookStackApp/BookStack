...
@section('body')
    ...
    <main class="content-wrap card">
     ...
     @if(auth()->check())
    <div style="display: flex; justify-content: flex-end; margin-top: 20px; position: relative; z-index: 9999; pointer-events: auto;">
        @if($isCompleted)
            <button class="button outline small" 
                    type="button" 
                    disabled 
                    style="background-color: #2e7d32; color: #fff; cursor: not-allowed;"
            >Completed
            </button>
        @else
            <button id="slack-notify-button" 
                    class="button outline small" 
                    type="button" 
                    style="background-color: #20134c; color: #fff; cursor: pointer;"
                    data-page-id="{{ $page->id }}"
                    data-page-name="{{ $page->name }}"
                    data-page-url="{{ $page->getUrl() }}">
                @icon('send') Mark as Complete
            </button>
        @endif
    </div>
    
    <script nonce="{{ $cspNonce ?? '' }}" src="{{ url('/custom_slack_notify.js') }}"></script>
...
@push('body-end')
    <script nonce="{{ $cspNonce ?? '' }}">
        document.addEventListener('DOMContentLoaded', function() {
            const btn = document.getElementById('slack-notify-button');
            if (btn) {
