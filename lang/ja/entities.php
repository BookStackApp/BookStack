...
return [
    ...
    'shelves_save' => '大分類を保存',
    'shelves_books' => 'この大分類のブック',
    'shelves_add_books' => 'この大分類にブックを追加',
    'shelves_drag_books' => '下にブックをドラッグしてこの大分類に追加',
    'shelves_empty_contents' => 'この大分類にはブックが割り当てられていません。',
    'shelves_edit_and_assign' => '大分類を編集してブックを割り当てる',
    'shelves_edit_named' => '大分類「:name」を編集',
    'shelves_edit' => '大分類を編集',
    'shelves_delete' => '大分類を削除',
    'shelves_delete_named' => '大分類「:name」を削除',
    'shelves_delete_explain' => "これにより、この大分類「:name」が削除されます。含まれているブックは削除されません。",
    ...
    'shelves_permissions_cascade_warning' => '大分類の権限は含まれる本には自動的に継承されません。これは、1つのブックが複数の大分類に存在する可能性があるためです。ただし、以下のオプションを使用すると権限を子ブックにコピーできます。',
    'shelves_copy_permissions_to_books' => 'ブックに権限をコピー',
    'shelves_copy_permissions' => '権限をコピー',
    'shelves_copy_permissions_explain' => 'これにより、この大分類の現在の権限設定を大分類に含まれるすべてのブックに適用します。有効にする前に、この大分類の権限への変更が保存されていることを確認してください。',
    'shelves_copy_permission_success' => '大分類の権限が:count個のブックにコピーされました',
    ...
    'tags_list_empty_hint' => 'タグはページエディタのサイドバーまたはブック、チャプター、大分類の詳細を編集しているときに割り当てることができます。',
    'attachments' => '添付ファイル',
    ...
    'convert_to_shelf_contents_desc' => 'このブックを同じ内容の新しい棚に変換できます。このブックに含まれるチャプターは新しいブックに変換されます。このブックにチャプター内にないページが含まれている場合、このブックは名前が変更され、そのようなページを含む新しい大分類の一部となります。',
    'convert_to_shelf_permissions_desc' => 'このブックに設定されているすべての権限は、新しい大分類と、独自の権限が適用されていないすべての新しい子ブックにコピーされます。大分類の権限はブックの場合のように、内部のコンテンツに自動的に継承されないことに注意してください。',
    'convert_book' => 'ブックを変換',
