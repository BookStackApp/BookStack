...
while [ $ELAPSED -lt $TIMEOUT ]; do
    MYSQL_HEALTH=$(docker-compose ps mysql --no-trunc 2>/dev/null | grep -c "healthy" || echo "0")
...
if [ $ELAPSED -ge $TIMEOUT ]; then
    ...
    docker-compose logs mysql >> "$MAIN_LOG" 2>&1
    exit 1
fi
...
cp /workspaces/BookStack/install.sh ./install.sh.test
...
if [ -d "/workspaces/BookStack/.eggs" ]; then
    warning "Found .eggs directory"
    GARBAGE_FOUND=$((GARBAGE_FOUND + 1))
fi

if find /workspaces/BookStack -maxdepth 2 -name "*.egg-info" -type d 2>/dev/null | grep -v ".git" | grep -q .; then
    log "Cleaning .egg-info directories..."
    find /workspaces/BookStack -maxdepth 2 -name "*.egg-info" -type d -exec rm -rf {} + 2>/dev/null || true
fi
...
docker-compose down >> "$MAIN_LOG" 2>&1
